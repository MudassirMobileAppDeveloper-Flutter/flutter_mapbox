...
class GestureListener(private val binaryMessenger: BinaryMessenger, private val messageChannelSuffix: String = "") {
  ...
  fun onTap(contextArg: MapContentGestureContext, callback: (Result<Unit>) -> Unit) {
    ...
    val channelName = "dev.flutter.pigeon.flutter_mapbox.GestureListener.onTap$separatedMessageChannelSuffix"
  ...
  fun onLongTap(contextArg: MapContentGestureContext, callback: (Result<Unit>) -> Unit) {
    val separatedMessageChannelSuffix = if (messageChannelSuffix.isNotEmpty()) ".$messageChannelSuffix" else ""
    val channelName = "dev.flutter.pigeon.flutter_mapbox.GestureListener.onLongTap$separatedMessageChannelSuffix"
    val channel = BasicMessageChannel<Any?>(binaryMessenger, channelName, codec)
    channel.send(listOf(contextArg)) {
      if (it is List<*>) {
        if (it.size > 1) {
  ...
  fun onScroll(contextArg: MapContentGestureContext, callback: (Result<Unit>) -> Unit) {
    val separatedMessageChannelSuffix = if (messageChannelSuffix.isNotEmpty()) ".$messageChannelSuffix" else ""
    val channelName = "dev.flutter.pigeon.flutter_mapbox.GestureListener.onScroll$separatedMessageChannelSuffix"
    val channel = BasicMessageChannel<Any?>(binaryMessenger, channelName, codec)
    channel.send(listOf(contextArg)) {
      if (it is List<*>) {
        if (it.size > 1) {
  ...
  fun onZoom(contextArg: MapContentGestureContext, callback: (Result<Unit>) -> Unit) {
    val separatedMessageChannelSuffix = if (messageChannelSuffix.isNotEmpty()) ".$messageChannelSuffix" else ""
    val channelName = "dev.flutter.pigeon.flutter_mapbox.GestureListener.onZoom$separatedMessageChannelSuffix"
    val channel = BasicMessageChannel<Any?>(binaryMessenger, channelName, codec)
    channel.send(listOf(contextArg)) {
      if (it is List<*>) {
        if (it.size > 1) {
...
abstract class AnnotationInteractionEventsStreamHandler : GestureListenersPigeonEventChannelWrapper<AnnotationInteractionContext> {
  companion object {
    fun register(messenger: BinaryMessenger, streamHandler: AnnotationInteractionEventsStreamHandler, instanceName: String = "") {
      var channelName: String = "dev.flutter.pigeon.flutter_mapbox.AnnotationInteractions._annotationInteractionEvents"
